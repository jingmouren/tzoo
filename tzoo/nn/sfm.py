...
class AdaSFM(nn.Module):
    ...
    def forward(self, X):
        ...
        ImS = torch.zeros(batch_size, self.hidden_size, self.hidden_size).to(self.device)
        ReS = torch.zeros(batch_size, self.hidden_size, self.hidden_size).to(self.device)
